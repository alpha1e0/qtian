...
| `TodoListPanel.vue` | 中间面板（受控）：list 名标题 + todo items 树；tree-toolbar 右侧图标按钮组（新建/筛选/收藏）；底部固定快捷输入框（路径前缀 + 输入框，Enter 创建，结尾 `#N` 控制优先级）。路径胶囊可点击锁定：锁定后路径固定，连续创建的子条目均落到锁定的父级下，不再因创建后选中漂移而需要重新点击父条目；再次点击取消锁定，回到"基于当前选中条目"的默认行为。切换 list 自动重置锁定态。筛选对话框优先级与状态均用 el-checkbox-group + el-checkbox 统一控件族：优先级横向一行排布、左侧语义色点颜色与标签文字同色（urgent=红 / important=琥珀 / normal=靛 / hint=灰，与 TodoItemRow priority-dot 同源），checkbox 选中色统一为 indigo accent 避免颜色过载；状态沿用竖向列表 | props: `listId`, `listName`, `isFavorite`；emit `select-item`, `select-list`, `toggle-status`, `delete-item`, `toggle-favorite` |
| `TodoItemRow.vue` | 单行 todo item，支持复选框、缩进、手动进度切换、行内删除（移至回收站） | emit `toggle-status`, `select`, `create-child`, `delete` |
| `TodoItemDetail.vue` | 右侧详情面板，分「基本信息 / 文档 / AI任务」三个 tab（与 sidebar 同款 radio-button toggle）。基本信息含：标题/状态/优先级/截止时间/描述/进度/手动进度；文档 tab 列出关联文档（`.doc-list` 列表样式，每行 icon+名称+更新时间+右侧删除按钮，点击行打开编辑器，删除按钮弹确认框调 `deleteDocument`）；AI任务含 task_prompt + Agent/LLM/额外 prompt 内联表单 + 运行/重跑/查看面板 | props: `itemId`；emit `updated`, `open-doc`, `run-task({agentName,llmConfigName,extraPrompt})`, `view-task` |
...
- **描述字段右键快捷创建待办条目**：描述字段（`todo_item.description` / `todo_list.description`）常被用户按行写好要点，逐行复制到快捷输入框效率低。在 3 处描述输入框（`TodoItemDetail` 内联 textarea、`TodoListDetail` 内联 textarea、`TodoDescriptionDialog` 对话框 textarea，共 4 个入口）支持「选中文本 + 右键 → 转换为待办条目」一键批量创建：
    - **触发位置**：上述 3 个组件内的 textarea。`TodoDescriptionDialog` 通过 `context` prop（`{ type: 'item' | 'list'; itemId: number | null; listId: number }`）由父组件（`TodoItemDetail` / `TodoListDetail`）传入，决定创建归属；内联 textarea 直接复用所在组件的 `itemId` / `listId` 上下文。
    - **事件委托方案**：`@contextmenu` **不挂在模板上**——`el-input` 用 `inheritAttrs: false`，模板 `@contextmenu` 不会 fallthrough 到内部 textarea；包 wrapper div 会破坏 `.el-form-item__content` 的 flex 布局；直接 `addEventListener` 到 textarea 元素则有时序耦合（ref 必须在绑定时就绪）。最终采用 **根/文档 capture 阶段事件委托**：`TodoItemDetail` / `TodoListDetail` 在 `mounted` 时给 `this.$el` 挂 `addEventListener('contextmenu', handler, true)`（capture 阶段），`TodoDescriptionDialog` 因 `append-to-body` teleport 内容到 body，在 `@opened` 时给 `document` 挂同样的 capture listener（`@closed` 移除）。handler `onDelegatedContextMenu(event)` 内部用 `this.$refs.descInput?.textarea ?? this.$refs.descInput?.$el?.querySelector('textarea')` 取到描述 textarea，再与 `event.target` 精确比对——只有右键发生在描述 textarea 上才进入后续流程，其他元素右键一律放行保留原生菜单。
    - **stopPropagation 关键点**：命中后必须 `event.preventDefault()` + `event.stopPropagation()`。capture 在根拦截后事件本会继续走 target → bubble → document；`TodoContextMenu` 的 close 监听器（document 级 `contextmenu` + `click`）恰好在 `$nextTick(attachListeners)` 后挂上，若放行原事件继续冒泡到 document，会立刻触发 `onDocClick → close` 把刚弹出的菜单关掉（Electron textarea 右键有此现象）。`stopPropagation` 阻止事件冒泡，`preventDefault` 阻止原生菜单。`TodoItemRow` 的行右键菜单不受此问题影响——它用模板 `@contextmenu`（bubble 阶段绑定），事件到达 document 前 handler 已执行完，但同样调了 `stopPropagation`（与 `TodoCategoryTree` 一致的既有模式）。
    - **菜单**：复用 `TodoContextMenu`，单项「转换为待办条目」（`Plus` 图标），fixed 跟随鼠标坐标（与 `TodoItemRow` 行右键菜单同款）。无选区时不 prevent / stop，让浏览器原生菜单弹出（保留复制 / 粘贴能力）。
    - **textarea 选区读取**：浏览器 `window.getSelection()` 对原生 `<textarea>` 不返回选区（历史行为），必须读 `event.target.selectionStart / selectionEnd` + `value.slice(start, end)`。共享 utility 抽到 `src/renderer/src/components/app-modules/todo-app/description-selection.js` 的 `getDescriptionSelection(event)`，无选区返回 `null`（调用方据此决定是否 prevent / stop）。
    - **解析规则**：`splitSelectionToTitles(text)` 纯函数（`src/main/core/services/app-modules/todo-app/todo-selection-create.ts`）按 `\n` split → 每行 `trim()` → 过滤空行；不解析 `#N` 控制符（避免描述里的 `#数字` 被误识别为优先级）；不去重（保留用户语义）。`\r\n` 由 `trim()` 清掉 `\r`。
    - **父级确定**：item 上下文 → `parent_id=itemId`（创建为该 item 的子条目）；list 上下文 → `parent_id=null`（创建为该 list 的根级条目）。
    - **批量 IPC**：新增 `TODO_CREATE_ITEMS_FROM_TEXT`，主进程 `TodoItemService.createFromText(text, { todo_list_id, parent_id })` 在单事务内循环 INSERT（参考 `bulkCreateForImport` 模式，但仅 title 必填、其余取默认值），每条同步 FTS；事务外若 `parent_id !== null` 调用 `recalcParentProgress(parent_id)` **一次**（避免 N 次重算）；返回创建好的 `TodoItem[]`（按创建顺序）。深度校验失败（`validateDepth` 抛错）与现有 `create` 一致。
    - **反馈**：textarea 触发组件（内联 / dialog）在 convert 命令处理后 emit `created(items)` 给父组件 → 父组件 emit `updated` → `TodoAppPage` 刷新 item 树（复用既有刷新路径）；同时 `ElMessage.success('已创建 N 个条目')`，右键菜单关闭。
    - **空选区 / 空文本行为**：`getDescriptionSelection` 返回 `null` 时不 prevent / stop，浏览器原生菜单弹出；空文本（全空行）落到主进程后 `splitSelectionToTitles` 返回 `[]`，`createFromText` 直接返回 `[]` 不写库、不抛错。
