...
| `TodoListPanel.vue` | 中间面板（受控）：list 名标题 + todo items 树；tree-toolbar 右侧图标按钮组（新建/筛选/收藏）；底部固定快捷输入框（路径前缀 + 输入框，Enter 创建，结尾 `#N` 控制优先级）。路径胶囊可点击锁定：锁定后路径固定，连续创建的子条目均落到锁定的父级下，不再因创建后选中漂移而需要重新点击父条目；再次点击取消锁定，回到"基于当前选中条目"的默认行为。切换 list 自动重置锁定态 | props: `listId`, `listName`, `isFavorite`；emit `select-item`, `select-list`, `toggle-status`, `delete-item`, `toggle-favorite` |
